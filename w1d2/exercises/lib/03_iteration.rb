  ...
  def bubble_sort!
  	...
  	while(!sorted)
	  	...
	  	for i in 0..self.length-2
	  		result = yield self[i], self[i+1] if block_given?
	  		if block_given? && result == 1
	  			self[i], self[i+1] = self[i+1], self[i]
	  			sorted = false
	  		elsif !block_given? && self[i] > self[i+1]
	  			self[i], self[i+1] = self[i+1], self[i]
	  			sorted = false
	  		end
	  	end
...
def subwords(word, dictionary)
	...
	for i in 0..dictionary.length-1
		subword = dictionary[i]
		for j in 0..word.length-1
			if word[j] == subword[counter-1]
				counter += 1
			else
				counter = 1
			end
...
class Array
  def my_map(&prc)
  	...
  	result = []
  	self.length.times do
  		result[i] = yield self[i] if block_given?
  		i+=1
  	end
  	result
  end
  ...
  def my_select(&prc)
  	...
  	result = []
  	self.length.times do
  		evaluate = yield self[i] if block_given?
  		if evaluate
	  		result << self[i]
	  		i+=1
	  	else
  ...
  def my_inject(&blk)
  	i=0
  	((self.length-1)/2 + 1).times do
  		if self[i+1].nil?
  			result = yield self[i+1], self[i] if block_given?
  			i+=2
  		else
  			result = yield self[i], self[i+1] if block_given?
  			i+=2
