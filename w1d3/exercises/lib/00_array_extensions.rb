  ...
  def two_sum
  	...
  	for i in 0..self.length-1
  		for j in 0..self.length-1
  			if i == j
  				#skip
  			elsif self[i] + self[j] == 0 && result == []
  				result << [i, j]
  			elsif self[i] + self[j] == 0
  				for k in 0..result.length-1
  					if result[k] == [i, j] || result[k] == [j, i]
  						break
  					elsif k == result.length-1 && result[k] != [i, j] && result[k] != [j, i]
  						result << [i, j]
  					end
  				end
  			end
...
class Array
  def median
  	...
  	if temp == []
  		...
  		temp[0]
  	elsif temp.length % 2 == 0
  		(temp[temp.length/2-1] + temp[(temp.length+1)/2])/2.0
  	else
