...
class String
  def caesar(shift)
  	for i in 0..self.length-1
  		self[i] = (self[i].ord + shift).chr
  		if self[i].ord > "z".ord
  			self[i] = (self[i].ord - 26).chr
  		end
  	end
  	self
  end
...
class Hash
  def difference(other_hash)
  	result = {}
  	self_keys = self.keys
  	input_keys = other_hash.keys
  	found = 0
  	for i in 0..self_keys.length-1
  		found = 0
  		for j in 0..input_keys.length-1
  			if self_keys[i] == input_keys[j]
  				found = 1
  				break
  			end
  		end
  		if found == 0
  			result[self_keys[i]] = self[self_keys[i]]
  		end
  	end
  	for i in 0..input_keys.length-1
  		found = 0
  		for j in 0..self_keys.length-1
  			if input_keys[i] == self_keys[j]
  				found = 1
  				break
  			end
  		end
  		if found == 0
  			result[input_keys[i]] = other_hash[input_keys[i]]
  		end
  	end
  	result
  end
  	...
  	if base == 10
  		...
  		if base == 16
  			if result.length%4 !=0
  				(4-result.length%4).times do
  					result << "0"
  				end
  			end
  			result.reverse!
  			n = 0
  			while(n+3 <= result.length-1)
	  			case result[n..n+3]
	  			when "0000"
	  				result_base16 << "0"
	  			when "0001"
	  			...
	  			when "1111"
	  				...
	  				puts "not a 4 digit binary number"
	  			end
	  			n+=4
	  		end
  		result_base16
